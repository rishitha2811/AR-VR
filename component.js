AFRAME.registerComponent('ar-reticle', { 
  ...
  init: function () { 
    ...
    this.xrHitTestSource = null;     this.viewerSpace = null; 
    this.refSpace = null; 
    ...
    sceneEl.renderer.xr.addEventListener('sessionstart', async () => { 
      ...
      session.addEventListener('select', () => { 
        ...
        model.object3D.position.copy(position);         model.setAttribute('visible', true); 
  ...
  tick: function () { 
    ...
    if (hitTestResults.length > 0) { 
      ...
      const matrix = new THREE.Matrix4();       matrix.fromArray(pose.transform.matrix); 
 
      const position = new THREE.Vector3();       position.setFromMatrixPosition(matrix); 
 
      this.el.object3D.position.copy(position);       this.el.setAttribute('visible', true); 
